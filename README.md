...
A Raspberry Pi bridge that exposes an IVT heat pump's sensors and thermostats to Apple HomeKit via the HAP-python library. Temperature readings and compressor state are read over CAN bus and updated every 2 minutes. Reading are saved in a SQLite database and can be visualized using Grafana. Works with IVT Premiumline HQ and Rego 1000 controller.
